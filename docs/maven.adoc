...
En la configuración previa, la ejecución se ha configurado también para que se incluya en el goal `verify`. Está configurado para que el informe se genere en la carpeta `target/site/checkstyle.html` y para que se muestre el resultado por consola.

El plugin jxr genera un informe de navegación por el código fuente, con enlaces a cada clase, método, etc. Se invoca con el goal: `jxr:jxr` y el resultado se genera en la carpeta `target/site/jxr/index.html`

Puesto que vamos a generar varios informes de análisis estático de código, y con el objetivo de que todos se generen correctamente, hay que llamar a los 3 plugins desde un único goal: `site`.	
...
La sección _Throubleshooting_ aborda los problemas comunes que pueden surgir de uso y configuración, incluyendo dificultades con la ejecución de pruebas y la generación de informes.

### VSCode no reinicia el cálculo de cobertura

Ocurre cuando, por ejemplo, obtienes un 100% de cobertura. Entonces, borras un test para comprobar que baja la cobertura, pero sigue mostrando el 100%.

El problema es el setting `java.test.config[].coverage.appendResult` que por defecto es `true` -> acumula resultados de ejecuciones anteriores en el mismo jacoco.exec, por eso siempre mostraba 100%.

Para resolverlo:

- Añadir `"appendResult": false` en `settings.json` — Ahora cada "Run with Coverage" sobrescribe los resultados en lugar de acumularlos.
- Recargar los datos: Ctrl+Shift+P → Developer: Reload Window
- Ejecutar _Run with Coverage_. Ya aparece la cobertura real
- Ya no te volverá a pasar.

Esto debería aparecer en el contenido de `.vscode/settings.json`:
